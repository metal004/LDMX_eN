...
myfile = TFile("pi0_invarmass_nominal_hists.root","RECREATE")
...
#    if(e[ph[0]]>=0. and cf_byslices500keV[0]*e[ph[0]]<100.):
...
def invar_mass_hcal_byslices500keV(e,px,py,pz,ph1,ph2):
    return np.sqrt(2.*hcal_byslices(e,ph1)*hcal_byslices(e,ph2)*(1.-cos_theta(px,py,pz,ph1,ph2)))
...
def invar_mass_1e1h_byslices500keV(ee,eh,px,py,pz,ph1,ph2):
    return np.sqrt(2.*cf_lw*ee[ph1[0]]*hcal_byslices(eh,ph2)*(1.-cos_theta(px,py,pz,ph1,ph2)))
...
#h_invarmass_hcal_pileup = TH1F("h_invarmass_hcal_pileup","#pi^{0} invariant mass hcal case",40,0,400)
h_invarmass_hcal_comb = TH1F("h_invarmass_hcal_comb","'Reco' #pi^{0} mass (hcal-hcal)",40,0,400) ##takes "visible/sim" energy

#h_invarmass_1e1h_pileup_1 = TH1F("h_invarmass_1e1h_pileup_1","#pi^{0} invariant mass 1e1h case",40,0,400)
h_invarmass_1e1h_comb_1 = TH1F("h_invarmass_1e1h_comb_1","#pi^{0} invariant mass 1e1h case",40,0,400)
#h_invarmass_1e1h_pileup_2 = TH1F("h_invarmass_1e1h_pileup_2","#pi^{0} invariant mass 1e1h case",40,0,400)
h_invarmass_1e1h_comb_2 = TH1F("h_invarmass_1e1h_comb_2","#pi^{0} invariant mass 1e1h case",40,0,400)
#h_invarmass_1e1h_pileup_tot = TH1F("h_invarmass_1e1h_pileup_tot","#pi^{0} invariant mass 1e1h case",40,0,400)
h_invarmass_1e1h_comb_tot = TH1F("h_invarmass_1e1h_comb_tot","'Reco' #pi^{0} mass (ecal-hcal)",40,0,400)

h_invarmass_ecal_lw = TH1F("h_invarmass_ecal_lw","'Reco' #pi^{0} mass (ecal-ecal)",40,0,400)
...
for entryNum in range(0,ana_tree.GetEntries()):
    ...
    if(len(ph_e)>ph1_idx[0] and len(ph_h)>ph1_idx[0] and len(ph_e)>ph2_idx[0] and len(ph_h)>ph2_idx[0]):
        ...
        # hcal
        if(ph_h[ph1_idx[0]]>1.5 and ph_e[ph1_idx[0]]==0 and ph_h[ph2_idx[0]]>1.5 and ph_e[ph2_idx[0]]==0):
#            h_invarmass_hcal_pileup.Fill(invar_mass_hcal_calibrated_mean(ph_h,ph_px,ph_py,ph_pz,ph1_idx,ph2_idx))
        ...
        # 1e1h 1
        if(ph_e_lw[ph1_idx[0]]>0 and ph_h[ph1_idx[0]]==0 and ph_h[ph2_idx[0]]>1.5 and ph_e_lw[ph2_idx[0]]==0):
#            h_invarmass_1e1h_pileup_1.Fill(invar_mass_1e1h_calibrated_mean_lw(ph_e_lw,ph_h,ph_px,ph_py,ph_pz,ph1_idx,ph2_idx))
        ...
        # 1e1h 2
        if(ph_e_lw[ph2_idx[0]]>0 and ph_h[ph2_idx[0]]==0 and ph_h[ph1_idx[0]]>1.5 and ph_e_lw[ph1_idx[0]]==0):
#            h_invarmass_1e1h_pileup_2.Fill(invar_mass_1e1h_calibrated_mean_lw(ph_e_lw,ph_h,ph_px,ph_py,ph_pz,ph2_idx,ph1_idx))
...
#h_invarmass_1e1h_pileup_tot = h_invarmass_1e1h_pileup_1.Clone()
#h_invarmass_1e1h_pileup_tot.Add(h_invarmass_1e1h_pileup_2)
...
h_invarmass_1e1h_comb_tot.Scale(1./h_invarmass_1e1h_comb_tot.Integral())
...
#h_invarmass_hcal_pileup.SetLineColor(1)
...
#h_invarmass_hcal_pileup.Draw("hist same")
#l1 = TLegend(0.6,0.6,0.95,0.85)
#l1.AddEntry(h_invarmass_hcal_pileup,"Requiring Hcal ph>0.5MeV")
#l1.AddEntry(h_invarmass_hcal_comb,"CF by energy slices with Hcal ph>0.5MeV")
#l1.Draw("hist same")
#print(h_invarmass_hcal_pileup.GetMean())
...
#h_invarmass_1e1h_pileup_tot.SetLineColor(1)
...
#h_invarmass_1e1h_pileup_tot.Draw("hist same")
#l1.Draw("hist same")
#print(h_invarmass_1e1h_pileup_tot.GetMean())
...
h_invarmass_ecal_lw.Draw("hist same")
...
l4.Draw("hist same")
c4.SaveAs("pi0_invarmass_strict.png")

myfile.WriteObject(c4,"pi0_invarmass_strict")
